/*  База данных для месенджера 'send_get' c каналами и группами, с правами доступа к чатам группам и каналам */


CREATE DATABASE IF NOT EXISTS send_get;
USE send_get;
...
CREATE TABLE users (
  ...
  username VARCHAR(100) UNIQUE NULL COMMENT 'для поиска по нику пользователя ---  @username',
...
-- соединение между  media content и  media_types
...
CREATE TABLE channels(
  ...
  link VARCHAR(100) UNIQUE NULL COMMENT 'Link автоматически генерируется для приватных каналов',
...
BEGIN
  RETURN CONCAT("https://sen.get/joinchat/", LEFT(UUID(), 22));
END //

-- генерирует ссылку для канала
...
CREATE TABLE user_roles (
  id INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
  user_role VARCHAR(30) NOT NULL COMMENT 'администратор, создатель, участник'
);


-- соединение между каналами и пользователями
...
CREATE TABLE chats(
  id SERIAL PRIMARY KEY,
  title VARCHAR(200) NULL COMMENT 'CONCAT(firstname, lastname) of user',
  photo_id BIGINT UNSIGNED NULL COMMENT 'user photo',
...
-- соединение между пользователем и чатом
...
-- соединение между пользователями и группами 
...
CREATE TABLE channels_messages (
  ...
  CONSTRAINT groups_users_media_id_fk FOREIGN KEY (media_id) REFERENCES media(id)
    ON DELETE SET NULL,
  INDEX channels_messages_from_auther_id_idx (from_auther_id) COMMENT 'поиск сообщения по автору',
  created_at DATETIME DEFAULT NOW(),
...
CREATE TABLE groups_and_chats_messages (
  ...
  CONSTRAINT groups_and_chats_messages_media_id_fk FOREIGN KEY (media_id) REFERENCES media(id)
    ON DELETE SET NULL,
  INDEX groups_and_chats_messages_from_user_id_idx (from_user_id),
  created_at DATETIME DEFAULT NOW(),
  edited_at DATETIME DEFAULT NOW() ON UPDATE NOW()
);
