...
-- Получение  сведений о правах пользователей в группе (поиск по id группы и статусе пользователя в user_roles)

SELECT user_id, user_permissions FROM groups_users WHERE group_id=[?] AND user_role=[?];

-- Получение админ-пользователей группы, имеющих право банить пользователей
